...
public final class SettingsEditor extends Stage {
    ...
    private void initSettingCategoryListView() {
        ...
        settingCategoryList.getSelectionModel().selectedItemProperty().addListener((observableValue, s, newValue) -> {
            sceneContainer.getChildren().removeFirst();
    ...
    private static class EnvironmentSettingsController {
        ...
        @FXML
        public TextField extensionsFilePathField;
        @FXML
        public Button browseExtensionsFilePathButton;
        @FXML
        public TextField pluginExtensionsFilePathField;
        @FXML
        public Button browsePluginExtensionsFilePathButton;
        @FXML
        public TextField collectionExtensionsFilePathField;
        @FXML
        public Button browseCollectionExtensionsFilePathButton;
        @FXML
        ...
        private void initUIElements() {
            ...
            final Setting<String> extensionsFilePathSetting = (Setting<String>) SettingsManager.getInstance().get("EXTENSIONS_FILE_PATH");
            extensionsFilePathField.setText(extensionsFilePathSetting.getValue());
            extensionsFilePathField.setPromptText(extensionsFilePathSetting.getInputFormatDescription());
            final Tooltip extensionsFilePathFieldTooltip = new Tooltip();
            extensionsFilePathFieldTooltip.setText(extensionsFilePathSetting.getDescription());
            extensionsFilePathField.setTooltip(extensionsFilePathFieldTooltip);
            extensionsFilePathField.focusedProperty().addListener((observableValue, oldValue, newValue) -> {
                if (!newValue) { // focus lost
                    if (extensionsFilePathSetting.verifyValue(extensionsFilePathField.getText().trim())) {
                        SettingsManager.getInstance().set("EXTENSIONS_FILE_PATH", extensionsFilePathField.getText().trim());
                    } else {
                        extensionsFilePathField.setText(extensionsFilePathSetting.getValue());
                    }
                }
            ...
            browseExtensionsFilePathButton.setOnAction(event -> {
                final File file = directoryChooser.showDialog(new Stage());
                if (file != null) {
                    SettingsManager.getInstance().set("EXTENSIONS_FILE_PATH", file.toString());
                    extensionsFilePathField.setText(file.toString());
                }
            });

            final Setting<String> pluginExtensionsFilePathSetting = (Setting<String>) SettingsManager.getInstance().get("PLUGIN_EXTENSIONS_FILE_PATH");
            pluginExtensionsFilePathField.setText(pluginExtensionsFilePathSetting.getValue());
            pluginExtensionsFilePathField.setPromptText(pluginExtensionsFilePathSetting.getInputFormatDescription());
            final Tooltip pluginExtensionsFilePathFieldTooltip = new Tooltip();
            pluginExtensionsFilePathFieldTooltip.setText(pluginExtensionsFilePathSetting.getDescription());
            pluginExtensionsFilePathField.setTooltip(pluginExtensionsFilePathFieldTooltip);
            pluginExtensionsFilePathField.focusedProperty().addListener((observableValue, oldValue, newValue) -> {
                if (!newValue) { // focus lost
                    if (pluginExtensionsFilePathSetting.verifyValue(pluginExtensionsFilePathField.getText().trim())) {
                        SettingsManager.getInstance().set("PLUGIN_EXTENSIONS_FILE_PATH", pluginExtensionsFilePathField.getText().trim());
                    } else {
                        pluginExtensionsFilePathField.setText(pluginExtensionsFilePathSetting.getValue());
                    }
                }
            ...
            browsePluginExtensionsFilePathButton.setOnAction(event -> {
                final File file = directoryChooser.showDialog(new Stage());
                if (file != null) {
                    SettingsManager.getInstance().set("PLUGIN_EXTENSIONS_FILE_PATH", file.toString());
                    pluginExtensionsFilePathField.setText(file.toString());
                }
            });

            final Setting<String> collectionExtensionsFilePathSetting = (Setting<String>) SettingsManager.getInstance().get("COLLECTION_EXTENSIONS_FILE_PATH");
            collectionExtensionsFilePathField.setText(collectionExtensionsFilePathSetting.getValue());
            collectionExtensionsFilePathField.setPromptText(collectionExtensionsFilePathSetting.getInputFormatDescription());
            final Tooltip collectionExtensionsFilePathFieldTooltip = new Tooltip();
            collectionExtensionsFilePathFieldTooltip.setText(collectionExtensionsFilePathSetting.getDescription());
            collectionExtensionsFilePathField.setTooltip(collectionExtensionsFilePathFieldTooltip);
            collectionExtensionsFilePathField.focusedProperty().addListener((observableValue, oldValue, newValue) -> {
                if (!newValue) { // focus lost
                    if (collectionExtensionsFilePathSetting.verifyValue(collectionExtensionsFilePathField.getText().trim())) {
                        SettingsManager.getInstance().set("COLLECTION_EXTENSIONS_FILE_PATH", tempFilePathField.getText().trim());
                    } else {
                        collectionExtensionsFilePathField.setText(collectionExtensionsFilePathSetting.getValue());
                    }
                }
            ...
            browseCollectionExtensionsFilePathButton.setOnAction(event -> {
                final File file = directoryChooser.showDialog(new Stage());
                if (file != null) {
                    SettingsManager.getInstance().set("COLLECTION_EXTENSIONS_FILE_PATH", file.toString());
                    collectionExtensionsFilePathField.setText(file.toString());
                }
            });
