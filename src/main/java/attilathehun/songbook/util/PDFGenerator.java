...
public final class PDFGenerator {
    ...
    public PDFGenerator(final CollectionManager manager) {
        ...
        try {
            ...
            throw new RuntimeException(Environment.IGNORED_EXCEPTION_MESSAGE);
        }
    }
    ...
    public String generatePreview(final Song s)  {
        ...
        new Thread(() -> {
            try (final BrowserFactory factory = BrowserFactory.getInstance()) {
                ...
                logger.error(e.getMessage(), e);
                Platform.runLater(() -> {
                    new AlertDialog.Builder().setParent(SongbookApplication.getMainWindow()).setTitle("Error").setIcon(AlertDialog.Builder.Icon.ERROR).setMessage("Something went wrong. See the log for more information.")
    ...
    private static class ExportWorker extends Task<Void> {
        ...
        protected Void call() throws Exception {
            ...
            updateProgress(1, 1);
            Thread.sleep(300);
            // TODO: wtf is this double nesting and double calling go check yourself man or don't code drunk
            Platform.runLater(() -> {
                dialog.close();
                Platform.runLater(() -> {
                    dialog.close();
